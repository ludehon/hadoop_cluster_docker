...
spark-launcher:
    image: bde/spark-app
    container_name: my-spark-app
    environment:
    - ENABLE_INIT_DAEMON=false
    links:
      - 'spark-master:spark-master'
    network_mode: docker-spark-master_default

docker build --rm -t bde/spark-app .
docker run --name my-spark-app -e ENABLE_INIT_DAEMON=false --link spark-master:spark-master --net docker-spark-master_default -d bde/spark-app

#### Create network
docker network create docker-spark-net
